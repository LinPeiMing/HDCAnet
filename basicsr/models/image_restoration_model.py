...
# from basicsr.losses.barlowTwinsloss2 import BT
...
#             z1, z2 = self.output.chunk(2, dim=1)
#             # l_contrast = self.cri_contrast(BT(self.output[:, :3], self.output[:, 3:]))
#             l_contrast = self.cri_contrast(BT(z1, z2))
...
#                 loss_dict['l_contrast'] = l_contrast
    ...
    def test(self):
        ...
        if hasattr(self, 'net_g_ema'):
            self.net_g_ema.eval()
            with torch.no_grad():
                # self.output, _, _, _ = self.net_g_ema(self.lq)
